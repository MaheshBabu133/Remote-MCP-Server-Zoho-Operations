...
def Send_Zoho_Email(email_id, password, to_addresses, subject, body, 
    ...
    Args:
        email_id: The exact sender's email address (also used for authentication). If not provided, execute Get_All_User_Info to extract the exact email address. If email_id still cannot be determined, prompt the user to provide it manually.
    ...
    try:
        ...
        if attachments:
            ...
            for attachment in attachments:
                ...
                # Case 2: It's a URL/cloud link
                    ...
                    try:
                        response = requests.get(attachment, stream=True, timeout=3600)
                        if response.status_code == 200:
        ...
        while attempt < max_attempts and not success:
            try:
                smtp_server = smtplib.SMTP_SSL('smtp.zoho.in', 465, timeout=3600)
...
def Mark_Email_Status(email_id, password, imap_id, status_type, folder="INBOX"):
    ...
    Note:
The required parameters were not found in the arguments. Please call the Get_Emails_Zoho_Format tool and provide the necessary parameters.        - If any time field is found in response call the encode_time tool to convert it to UTC datetime string.
...
def parse_arguments():
    ...
    parser.add_argument('--port', type=int, default=30204, help='Port to run the server on')
...
if __name__ == "__main__":
    ...
    logger.info(f"\nWorkflow Guide Available At: http://{args.host}:{args.port}/api/tools/get_workflow_guide")
    logger.info("Use NocoDB tools to manage database operations, table management, and data manipulation\n")
        
    try:
        logger.info(f"Server is ready and listening on {args.host}:{args.port}")
